...
**解决**：`statusline.py` 不显示成本（谁都知道自己用的是什么价格），而是显示**会话压缩次数**——`total_input_tokens / 600K`。
...
> 🎯 **适用范围：所有模型。** 这个功能追踪的是 Claude Code 自身的上下文压缩行为，不依赖模型定价。不管后端是 DeepSeek、Anthropic 还是 OpenAI 兼容接口，只要 Claude Code 自动压缩，数字就涨。**所有 Claude Code CLI 用户都能直接用。**
...
| 状态行 | 只显示成本（Anthropic 定价） | **实时压缩次数**（记忆退化预警，所有模型通用） |
...
| ⚡ **Token 优化** | RTK shell 输出精简（实测 55%） · 缓存命中率优化（50%→90%+） · 子 Agent pro 模型 · 状态行压缩计数（所有模型通用） |
| 📊 **状态行** | 实时压缩次数追踪——告诉你模型还记得多少、什么时候该新开会话 |
...
- **状态行压缩次数**：比价格更有用的指标——告诉你模型还记得多少，压缩×5+ 建议新开会话
